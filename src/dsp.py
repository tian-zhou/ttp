...
class DSP:
    def __init__(self):
        ...
        self.fb = FilterBank(full = False, thesis_version = False)
    ...
    def clip_outlier(self, x):
        ...
        # x[19] = max(2200, min(1200, x[19])) # Epoc_GYROX, added to address subject 1,2,3's noise data
        # x[20] = max(1900, min(1400, x[20])) # Epoc_GYROY, added to address subject 1,2,3's noise data
    ...
    def encode_features_offline(self, z_buf):
        """
        ...
        s_time = time()
        xf = self.fb.applyFilter(z_buf, filterName = 'all', GaussBlur = False)
        print "applyFilter takes %.3f seconds" % (time()-s_time)

        s_time = time()
        en_buf, encode_feat_names = self.fb.convertFilteredToMatrix(xf, self.good_feat_names)
        print "convertFilteredToMatrix takes %.3f seconds" % (time()-s_time)

        return en_buf, encode_feat_names

    def encode_features_online(self, z_buf):
        """
        z.buf.shape: (3, num_features)
        the 1st dimension is the temporal dimension
        the 2nd dimension is the feature dimension
        apply filters on each column of z_buf to create encoded versions
        return en_buf, encode_feat_names
        ...
        """
        z_buf = np.array(z_buf)
        en_buf, encode_feat_names = self.fb.apply_filter_online(z_buf, self.good_feat_names)
        return en_buf, encode_feat_names
